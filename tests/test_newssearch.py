"""
These tests cover DuckDuckGo news searches.
"""

import pytest as pytest
from pages.result import DuckDuckGoResultPage
from pages.search import DuckDuckGoSearchPage
from pages.newsresults import DuckDuckGoNewsResultsPage
from selenium.webdriver.common.keys import Keys


@pytest.mark.parametrize("phrase", ["panda"])
def test_duckduckgo_news_search(browser, phrase):
    search_page = DuckDuckGoSearchPage(browser)
    result_page = DuckDuckGoResultPage(browser)
    news_page = DuckDuckGoNewsResultsPage(browser)
    ...
    titles = news_page.result_link_titles()
    matches = [t for t in titles if phrase.lower() in t.lower()]
    assert len(matches) > 0

    # Then the search result title contains "phrase"
    assert phrase in news_page.title()
